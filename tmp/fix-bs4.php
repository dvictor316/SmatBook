...
// ── 7. Vendor credit CSS after overdraft-tag block ───────────────────────────
$c = str_replace(
'.bs-overdraft-tag {
...
// ── 8. Vendor credit badge in current assets account name cell ───────────────
// 32 spaces before <tr, 36 before <td> (confirmed from file inspection)
$c = str_replace(
...
// ── 9. Debug panel after the last @endif in the equation section ─────────────
$c = str_replace(
'            @endif
...
if ($allOk) {
    ...
    echo "\nAll 3 blocks applied. Lines: " . substr_count($c, "\n") . "\n";
} else {
    // Show diagnostic for failed blocks
    if ($count8 === 0) {
        echo "\nDiagnostic for block 8 — searching for tr + td combination:\n";
        $pos = strpos($c, 'bs-line bs-line-indented');
        // find all HTML occurrences (not CSS)
        $offset = 0;
        while (($pos = strpos($c, 'bs-line bs-line-indented', $offset)) !== false) {
            $chunk = substr($c, $pos - 60, 300);
            if (strpos($chunk, '<tr') !== false) {
                echo "Found HTML occurrence at pos $pos:\n";
                echo "---BEGIN---\n";
                // show with space markers
                echo str_replace(["\n", " "], ["↵\n", "·"], $chunk) . "\n";
                echo "---END---\n";
            }
            $offset = $pos + 1;
