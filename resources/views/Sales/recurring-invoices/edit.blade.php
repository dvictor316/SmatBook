...
function addRow(data = {}) {
    ...
    tr.innerHTML = `
        <td><input type="text"   name="items[${i}][product_name]" class="form-control form-control-sm" value="${data.product_name||''}" required>
...
function recalc() {
    ...
    document.querySelectorAll('#itemsBody tr').forEach(tr => {
        ...
        tr.querySelector('.row-total').textContent = (line + t - d).toFixed(2);
...
@php $existingItems = $template->items ?? []; @endphp
