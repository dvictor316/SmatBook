                            ...
                            <table class="table hotel-type-table align-middle mb-0">
                                ...
                                <tbody>
                                    @forelse($entries as $item)
                                        <tr><td><span class="hotel-status-chip gold">Room {{ $item->folio?->stay?->room?->room_number ?? 'N/A' }}</span></td><td>{{ $item->folio?->customer?->customer_name ?? $item->folio?->customer?->name ?? 'N/A' }}</td><td>{{ $item->description }}</td><td>{{ $item->quantity }}</td><td>{{ number_format((float) ($item->line_total ?? $item->amount ?? 0), 2) }}</td><td>{{ optional($item->created_at)->format('d M Y H:i') }}</td><td><a class="btn btn-sm btn-outline-dark" target="_blank" rel="noopener" href="{{ route('hotel.folios.items.receipt', $item) }}">Receipt</a></td></tr>
                                    @empty
