    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="content-page-header">
                ...
                <p class="text-muted mb-0">Update pricing rules and item prices for this list.</p>
            </div>
        </div>
                ...
                <form method="POST" action="{{ route('price-lists.update', $priceList) }}">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="currency" class="form-control" value="{{ old('currency', $priceList->currency) }}" maxlength="3" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Discount Type</label>
                            <select name="discount_type" class="form-select">
                                <option value="">None</option>
                                <option value="percentage" @selected(old('discount_type', $formData['discount_type']) === 'percentage')>Percentage</option>
                                <option value="fixed" @selected(old('discount_type', $formData['discount_type']) === 'fixed')>Fixed</option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Discount Value</label>
                            <input type="number" step="0.01" min="0" name="discount_value" class="form-control" value="{{ old('discount_value', $formData['discount_value']) }}">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3 d-flex align-items-end">
                            <div class="form-check me-3">
                                <input class="form-check-input" type="checkbox" name="is_default" value="1" id="is_default" @checked(old('is_default', $priceList->is_default ?? false))>
                                <label class="form-check-label" for="is_default">Default</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_active" value="1" id="is_active" @checked(old('is_active', $priceList->is_active))>
                                <label class="form-check-label" for="is_active">Active</label>
                            </div>
                        </div>
                        <div class="col-12">
                            <label class="form-label">Notes</label>
                            <textarea name="notes" class="form-control" rows="3">{{ old('notes', $formData['notes']) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="price-items-table">
                            ...
                            <tbody>
                                @php
                                    $existingItems = $priceList->items->map(function ($item) {
                                        return [
                                    ...
                                    $items = old('items', $existingItems ?: [['product_id' => '', 'price' => '', 'min_quantity' => 1]]);
                                @endphp
