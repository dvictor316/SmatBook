...
<style>
    ...
    .sa-section-head h4 { margin:0; color:#061b33; font-weight:900; }
    ...
    .sa-directory-card .eyebrow { color:#d4a23a; text-transform:uppercase; letter-spacing:.12em; font-size:12px; font-weight:900; }
    ...
    .sa-room-tile .room-no { font-size:38px; font-weight:900; color:#061b33; line-height:1; }
    ...
    .sa-pill-event { display:inline-flex; align-items:center; border-radius:999px; padding:7px 11px; color:#fff; background:#2563eb; font-weight:900; font-size:12px; }
    ...
    .sa-big-avatar { width:64px; height:64px; border-radius:50%; background:#0b2f54; color:#fff; display:grid; place-items:center; font-size:25px; font-weight:900; }
    ...
    .sa-payment-pad div { min-height:82px; border:1px solid #d8e2ee; border-radius:14px; background:#fff; display:grid; place-items:center; text-align:center; font-weight:900; }
...
<div class="page-wrapper sa-hotel">
    <div class="content container-fluid">
        ...
        @elseif(in_array($panel, ['tenants','properties'], true))
            <section class="sa-dash-panel">
                <div class="sa-section-head">
                    <div>
                        <small class="text-warning fw-bold">{{ $panel === 'tenants' ? 'TENANT CONTROL' : 'PROPERTY DIRECTORY' }}</small>
                ...
                <section class="sa-dash-panel">
                    ...
                    <div class="sa-room-wall">
                        @forelse($panelRows as $row)
                            ...
                            <article class="sa-room-tile {{ $state }}"><div><div class="d-flex justify-content-between"><span class="fw-bold">{{ ucfirst(str_replace('_',' ', $state)) }}</span><span class="badge bg-light text-dark">{{ ucfirst((string)($r['housekeeping_status'] ?? 'clean')) }}</span></div><div class="room-no">{{ $r['room_number'] ?? ('#'.($r['id'] ?? '-')) }}</div><strong>Property {{ $r['property_id'] ?? '-' }}</strong></div><small class="text-muted">Company {{ $r['company_id'] ?? '-' }} - Type {{ $r['room_type_id'] ?? '-' }}</small></article>
                        @empty
                            @foreach(['101','102','103','104','201','202','203','204'] as $roomNo)
                                <article class="sa-room-tile {{ $loop->iteration % 4 === 0 ? 'reserved' : ($loop->iteration % 3 === 0 ? 'occupied' : 'available') }}"><div><span class="fw-bold">Preview</span><div class="room-no">{{ $roomNo }}</div><strong>Hotel room board</strong></div><small class="text-muted">Live rooms appear after tenant setup.</small></article>
                            @endforeach
                        @endforelse
                    </div>
                </section>
        ...
        @elseif($panel === 'room_types')
            <section class="sa-dash-panel"><div class="sa-section-head"><div><small class="text-warning fw-bold">ROOM CATALOGUE</small><h4>Room Types & Rate Classes</h4><p>Capacity, base rates and tenant room-type setup.</p></div><span class="btn btn-outline-primary disabled">{{ $panelRows->count() }} types</span></div><div class="sa-directory-grid">@forelse($panelRows as $row)@php $r=$rowArray($row); @endphp<article class="sa-directory-card"><span class="eyebrow">Room Type</span><h5>{{ $r['name'] ?? ('Type #'.($r['id'] ?? '-')) }}</h5><p class="text-muted">Company {{ $r['company_id'] ?? '-' }} - Property {{ $r['property_id'] ?? '-' }}</p><div class="d-flex justify-content-between"><span>Occupancy</span><strong>{{ $r['max_occupancy'] ?? '-' }}</strong></div><div class="d-flex justify-content-between"><span>Base Rate</span><strong>{{ $money($r['base_rate'] ?? 0) }}</strong></div></article>@empty<div class="sa-empty">No room types found yet. Tenant room classes will appear here.</div>@endforelse</div></section>
        @elseif(in_array($panel, ['room_calendar','reservations'], true))
            <section class="sa-dash-panel">
                <div class="sa-section-head"><div><small class="text-warning fw-bold">{{ $panel === 'room_calendar' ? 'ROOM CALENDAR' : 'RESERVATION REGISTER' }}</small><h4>{{ $panel === 'room_calendar' ? 'Reservation Timeline' : 'Bookings Operations Board' }}</h4><p>Arrival, departure, room assignment, booking status and deposits.</p></div><a class="btn btn-outline-primary" href="{{ route('super_admin.hotels.index', ['panel'=>'check_in', 'company_id'=>$selectedCompanyId]) }}">Check-In Queue</a></div>
                <div class="sa-timeline"><table class="table table-bordered align-middle"><thead><tr><th>Reservation</th><th>Guest</th><th>Room</th><th>Arrival</th><th>Departure</th><th>Nights</th><th>Status</th><th>Deposit</th></tr></thead><tbody>@forelse($panelRows as $row)@php $r=$rowArray($row); @endphp<tr><td><span class="sa-pill-event {{ ($r['status'] ?? '') === 'confirmed' ? 'green' : 'gold' }}">{{ $r['reservation_number'] ?? ('#'.($r['id'] ?? '-')) }}</span></td><td>Guest #{{ $r['customer_id'] ?? '-' }}</td><td>{{ $r['room_id'] ?? 'Unassigned' }}</td><td>{{ $r['arrival_date'] ?? '-' }}</td><td>{{ $r['departure_date'] ?? '-' }}</td><td>{{ $r['nights'] ?? '-' }}</td><td><span class="badge {{ $statusBadge($r['status'] ?? 'reserved') }}">{{ ucfirst(str_replace('_',' ', (string)($r['status'] ?? 'reserved'))) }}</span></td><td>{{ $money($r['deposit_received'] ?? 0) }}</td></tr>@empty<tr><td colspan="8"><div class="sa-empty">No reservations found yet. Live tenant bookings will populate this PMS calendar/register.</div></td></tr>@endforelse</tbody></table></div>
            </section>
        @elseif($panel === 'availability')
            <section class="sa-dash-panel"><div class="sa-section-head"><div><small class="text-warning fw-bold">AVAILABILITY SEARCH</small><h4>Rooms Available For Sale</h4><p>Read-only platform availability signals for selected tenant scope.</p></div><span class="btn btn-success disabled">{{ $availableRooms }} available</span></div><div class="sa-directory-grid">@forelse($panelRows as $row)@php $r=$rowArray($row); @endphp<article class="sa-directory-card"><span class="eyebrow">Demand Signal</span><h5>{{ $r['reservation_number'] ?? ('Reservation #'.($r['id'] ?? '-')) }}</h5><p class="text-muted">Room {{ $r['room_id'] ?? 'Unassigned' }} - Type {{ $r['room_type_id'] ?? '-' }}</p><div class="d-flex justify-content-between"><span>Arrival</span><strong>{{ $r['arrival_date'] ?? '-' }}</strong></div><div class="d-flex justify-content-between"><span>Departure</span><strong>{{ $r['departure_date'] ?? '-' }}</strong></div></article>@empty<div class="sa-empty">No active availability demand found. Available rooms are still tracked above.</div>@endforelse</div></section>
