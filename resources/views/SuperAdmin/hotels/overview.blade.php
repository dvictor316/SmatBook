    ...
    <div class="content container-fluid">
        ...
        @elseif($panel === 'rooms')
            <div class="sa-room-admin">
                ...
                <section class="sa-panel p-3"><div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3"><div><h4 class="mb-1">Platform Room Inventory Board</h4><p class="text-muted mb-0">Super Admin mirror of tenant room stock, housekeeping state, operational state and property mapping.</p></div><span class="badge bg-light text-dark">{{ $panelRows->count() }} loaded</span></div>@if($panelRows->isEmpty())<div class="sa-empty">No rooms found yet. The rack will show each tenant room as a colored operational tile.</div>@else<div class="sa-room-rack">@foreach($panelRows as $row)@php $r=$rowArray($row); $state=(string)($r['operational_status'] ?? 'available'); @endphp<div class="sa-room {{ $state }}"><div class="d-flex justify-content-between small"><span>{{ ucfirst(str_replace('_',' ', $state)) }}</span><span>⋮ □</span></div><div class="sa-room-num">{{ $r['room_number'] ?? ('#'.($r['id'] ?? '-')) }}</div><strong>Property {{ $r['property_id'] ?? '-' }}</strong><div class="small text-muted">Company {{ $r['company_id'] ?? '-' }} · Type {{ $r['room_type_id'] ?? '-' }}</div><div class="small mt-2">Housekeeping: {{ ucfirst((string)($r['housekeeping_status'] ?? 'clean')) }}</div></div>@endforeach</div>@endif</section>
            </div>
        ...
        @elseif(in_array($panel, ['folios','deposits','revenue','hotel_transactions'], true))
            <section class="sa-folio-register">
                <aside class="sa-folio-rail"><small>{{ strtoupper($panelTitle) }}</small><h3>{{ $money($outstandingReceivables) }}</h3><p>Platform cashier register for tenant hotel folios, deposits, revenue and transactions.</p><a href="{{ route('super_admin.hotels.index', ['panel'=>'folios', 'company_id'=>$selectedCompanyId]) }}"><span>Folios</span><strong>Open</strong></a><a href="{{ route('super_admin.hotels.index', ['panel'=>'deposits', 'company_id'=>$selectedCompanyId]) }}"><span>Deposits</span><strong>Trace</strong></a><a href="{{ route('super_admin.hotels.index', ['panel'=>'services', 'company_id'=>$selectedCompanyId]) }}"><span>Service Centers</span><strong>Audit</strong></a><a href="{{ route('super_admin.hotels.index', ['panel'=>'revenue', 'company_id'=>$selectedCompanyId]) }}"><span>Revenue</span><strong>Review</strong></a></aside>
