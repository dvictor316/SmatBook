                    ...
                    <div class="sa-room-wall">
                        ...
                        @empty
                            @foreach(['101','102','103','104','201','202','203','204'] as $roomNo)
                                <article class="sa-room-tile {{ $loop->iteration % 4 === 0 ? 'reserved' : ($loop->iteration % 3 === 0 ? 'occupied' : 'available') }}"><div><span class="fw-bold">Preview Room</span><div class="room-no">{{ $roomNo }}</div><strong>{{ $loop->odd ? 'Deluxe King' : 'Executive Suite' }}</strong></div><small class="text-muted">Tenant adds photos and panorama under Hotel > Rooms.</small></article>
                            @endforeach
                        @endforelse
                    </div>
                </section>
